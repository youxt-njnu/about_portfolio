// https://www.bilibili.com/list/watchlater?oid=697633525&bvid=BV1Km4y127uP&spm_id_from=333.999.top_right_bar_window_view_later.content.click
//  1. 粘贴进来
//  2. 替换fragColor为gl_FragColor
//  3. 替换mainImage函数名为main
//  4. 修改main入参
//  5. 修改fragCoord为gl_FragCoord
//  6. main中使用fragCoord的xy即可，这样进行相减操作
//  7. 分析uniform，哪些是必须的(iTime, iResolution), 写入glsl和index.jsx中
//  8. 修改获取uv的方式
...
// current (if MLB down), zw: click uniform samplerXX iChannel0..3;          //
// input channel. XX = 2D/Cube uniform vec4      iDate;                 //
// (year, month, day, time in seconds) uniform float     iSampleRate;
